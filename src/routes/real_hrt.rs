...
async fn get_realtime_bpm(
    ...
    let real_sql = "SECECT ROUND(bpm) as bpm FROM bpmdata WHERE email = $1 ORDER BY time DESC FETCH FIRST 1 ROW ONLY";
    match client.query_one(real_sql, &[&params.email]).await {
        Ok(row) => {
            let bpm: i32 = row.get("bpm"); //쿼리 결과에서 bpm 값을 가져옴
            Json(BpmValue { bpm }).into_response() //JSON 응답 생성
        }
        Err(err) => {
            eprintln!("Error querying realtime BPM data: {:?}", err); //오류 로그 출력
            (

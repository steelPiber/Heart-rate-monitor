...
async fn get_year_bpm(Query(params): Query<YearQuery>, client: Arc<Client>) -> impl IntoResponse {
    let min_sql = "SELECT ROUND(AVG(bpm)) AS avg_bpm FROM bpmdata WHERE email = :Email AND time > (SELECT MAX(time) - INTERVAL '1' YEAR FROM bpmdata WHERE email = :Email)";
    match client.query_one(min_sql, &[&params.email]).await {
        ...
        Err(err) => {
            eprintln!("Error querying year BPM data: {:?}", err); //오류 로그 출력
            (
...
pub fn create_routes(client: Arc<Client>) -> Router {
    Router::new().route("/", get(move |query| get_year_bpm(query, client.clone())))
}
